...
2. if you modify the docker volumes to bind mounts in the compose file for persistent storage, please check the troubleshooting section below if you get read/write permission denied or file not found errors. if you are using this on `Windows` with `WSL`, Please make sure to put the windows filesystem path instead of linux specific ones on the left hand side.
